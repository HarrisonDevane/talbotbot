...
class SelfPlayGameWorker:
    ...
    """
    def __init__(self, logger, player_1, player_2, model_config, self_play_config):
        self.logger = logger
        ...
        while not self.game_over:
            ...
            if move_count < max_opening_moves:
                try:
                    ...
                    move, policy_vector, root_value = player.get_move(current_board, move_count, search_depth, None)
                    max_opening_moves = 0
            else:
                move, policy_vector, root_value = player.get_move(current_board, move_count, search_depth, None)
    ...
    def run_eval_loop(self, game_number):
        ...
        while not self.game_over:
            ...
            opponent_color = not self.current_turn
            last_opponent_move = last_moves[opponent_color]
            ...
            if move_count < max_opening_moves:
                try:
                    ...
                    move, policy_vector, root_value = player.get_move(current_board, move_count, search_depth, last_opponent_move)
                    max_opening_moves = 0
            else:
                move, policy_vector, root_value = player.get_move(current_board, move_count, search_depth, last_opponent_move)
                
            last_moves[self.current_turn] = move
            self.board.push(move)
            self.current_turn = not self.current_turn
            self.logger.info(f"Game {game_number} - Move made: {move.uci()}")
            move_count += 1
