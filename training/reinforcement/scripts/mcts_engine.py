...
class MCTSEngine:
    ...
    def _add_dirichlet_noise(self, policy_probs):
        ...
        # Create a vector of noise from a Dirichlet distribution
        ...
        # Blend the original policy and the noise
        noisy_policy = (1 - self.dirichlet_epsilon) * policy_probs + self.dirichlet_epsilon * dirichlet_noise

        self.logger.info(f"Added Dirichlet noise to root node")
    ...
    def set_new_root(self, board: chess.Board, last_move: chess.Move):
        ...
        if last_move and last_move in self.root.children:
            ...
            if self.root.is_expanded and self.root.prior_probabilities is not None:
                self.logger.debug("Applying Dirichlet noise to the new root's existing policy.")
                self.root.prior_probabilities = self._add_dirichlet_noise(self.root.prior_probabilities)

            self.logger.info(f"MCTS root advanced by sequence: {last_move.uci()}")
    ...
    def run_simulations(self, search_depth):
        ...
        # Log root children stats at the end of run_simulations only if logging level = info
        if self.logger.isEnabledFor(logging.INFO) and self.root and self.root.children:
