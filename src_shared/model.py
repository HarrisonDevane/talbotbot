...
class ResidualBlock(nn.Module):
    # REMOVED: dropout_rate_conv parameter
...
class ChessAIModel(nn.Module):
    # REMOVED: dropout_rate_conv, dropout_rate_fc, and dropout_conv_start_block parameters
    def __init__(self, num_input_planes: int, num_residual_blocks: int = 68, num_filters: int = 128):
        ...
        # Stack multiple Residual Blocks, all now without internal dropout
        residual_blocks_list = [ResidualBlock(num_filters) for _ in range(num_residual_blocks)]
